...
export default function EventDetails() {
  ...
  const [attendees, setAttendees] = useState<AttendeeForm[]>([{ firstName: '', lastName: '', email: '', phone: '' }]);
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        ...
        setEvent(data);
        setAttendees(Array(1).fill({ firstName: '', lastName: '', email: '', phone: '' }));
      } catch (error) {
  ...
  const handleQuantityChange = (newQuantity: number) => {
    if (newQuantity < 1 || (event && newQuantity > event.remainingSlots)) return;
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  ...
                  <div className="mt-1 flex items-center space-x-3">
                    ...
                    <button
                      ...
                      onClick={() => handleQuantityChange(quantity + 1)}
                      className="px-3 py-2 border rounded-md hover:bg-gray-100"
                      disabled={quantity >= event.remainingSlots || submitting}
                    >
                      +
                    </button>
                  </div>
                ...
                <div className="pt-4 border-t border-gray-200">
                  ...
                  <div className="flex items-center justify-between text-lg font-bold">
                    <span>Total</span>
                    <span>
                      {event.isPaid ? `$${(event.price! * quantity).toFixed(2)}` : 'Free'}
                    </span>
                  </div>
                </div>
