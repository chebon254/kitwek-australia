...
export async function POST(request: Request) {
  try {
    ...
    const uploadCommand = new PutObjectCommand({
      ...
      ACL: 'private', // Private for welfare documents
    });

    await s3Client.send(uploadCommand);

    // Generate private URL - for welfare documents, you might want to implement
    // a separate route to serve these files with proper authentication
